...
print('학습용 데이터셋 점수 : {:.2f}'.format(lr.score(X_train,y_train)))
print('테스트용 데이터셋 점수 : {:.2f}'.format(lr.score(X_test,y_test)))
#%% 라쏘 모델 확인
...
print("훈련 데이터 세트 점수 : {:.2f}".format(lasso01.score(X_train, y_train)))
print("테스트 데이터 세트 점수 : {:.2f}".format(lasso01.score(X_test, y_test)))
...
plt.plot(lasso.coef_, "bs", label="Lasso alpha=10")
